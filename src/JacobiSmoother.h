...
public:
  ...
  JacobiSmoother(StiffnessMatrix &M, int Step, double w, bool ZeroStart,
                 Eigen::VectorXd &m1)
      : _mat(M), _idia(M.Diagonal()), _step(Step), _w(w), _zeroStart(ZeroStart),
        r(m1) {}

  /**
   * @brief This constructor copies the configuration of the old Jacobi solver.
   * ZeroStart must be extra set.
   *
  ...
  JacobiSmoother(JacobiSmoother &old, bool ZeroStart)
      : _mat(old._mat), _idia(old._idia), _step(old._step), _w(old._w),
        _zeroStart(ZeroStart), r(old.r) {}

  virtual ~JacobiSmoother() {}

  int Solve(Eigen::VectorXd &b, Eigen::VectorXd &x);

  virtual const std::string Label() const { return "Jacobi Smoother"; }
