...
/*! PrintVector is a helper class to display vectors with different orderings of
 * the items.
 *
 * It is used for debung reason. It is not testet in parallel.
 */
template <class T> class PrintVector {
public:
  /**
   * @brief Constructor
   *
   ...
   */
  void NormalToOctgrid(const Eigen::VectorXd &normal, Eigen::VectorXd &oct,
                       t_index block) {
    t_index ind;
   ...
   */
  void OctgridToNormal(const Eigen::VectorXd &oct, Eigen::VectorXd &normal,
                       int block) {
