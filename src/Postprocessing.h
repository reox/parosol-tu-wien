...
/*! The PostProcess Class computes the von mises stress and the strain energy
 * density from the resulting displacement of the computation.
 */

template <class Grid> class PostProcess {
public:
  ...
  void ComputeStressAndStrain(Eigen::VectorXd &disp, Eigen::VectorXd &VonMises,
                              Eigen::VectorXd &SED, Eigen::VectorXd &eff,
    ...
    for (_grid.initIterateOverElements(); _grid.TestIterateOverElements();
      ...
      Element_Stress(_matprop, NumMaterialProps, NumNodesPerElement,
                     NumDofsPerElement, Dimension, NumGaussPoints, SSMatrixSize,
                     coord, &xpref[0], strainbuf, stressbuf, &sigma, &theta);
