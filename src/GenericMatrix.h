...
//! A class to perform an matrix-vector product for elasticity problem based on
//! grid
...
template <class Grid> class GenericMatrix : public StiffnessMatrix {
public:
  GenericMatrix(Grid &grid);

  ~GenericMatrix();

  /** The Application is done with respect to the boundary condition. The matrix
   * is not assembled This function needs communication.
   *
  ...
  /** This application of the matrix will not apply the boundary condition. It
   * uses the indefinite matrix. This function needs communication.
   *
   *  @param [in] x input vector
   *  @param [out] y output vector
   */
  ...
  inline virtual const std::string Label() const { return ("Matrix"); }
  ...
  BoundaryCondition &GetBC() { return _grid.bc; }

  //! Get the local Number of Dofs
  t_index GetNrDofs() { return _mydofs; }

  /** Computes the dot product of the vectors based on the grid
   *  @param a first input vector
   *  @param b second input vector
   */
  double dot(Eigen::VectorXd &a, Eigen::VectorXd &b) { return _grid.dot(a, b); }

  friend std::ostream &operator<<(std::ostream &stream,
                                  const GenericMatrix &mat) {
...
GenericMatrix<Grid>::GenericMatrix(Grid &grid)
  ...
  Stiffness_Matrix(_matprop, NumMaterialProps, NumNodesPerElement,
                   NumDofsPerElement, Dimension, NumIntegrationPoints,
                   SSMatrixSize, coord,
                   tmpstiff); // TODO: not sure if this works
...
template <class Grid> GenericMatrix<Grid>::~GenericMatrix() {
...
template <class Grid>
int GenericMatrix<Grid>::Apply_NoResetBoundaries(Eigen::VectorXd &x,
                                                 Eigen::VectorXd &y) {
  ...
  for (_grid.initIterateOverElements(); _grid.TestIterateOverElements();
       _grid.IncIterateOverElements()) {
    _grid.GetNodalDisplacementsOfElement(x, xpref);

    nr_ele++;
    // decarbenz(res.data(), xpref.data());
    res = (*_stiffnessmatrix) * xpref;
    _grid.SumInToNodalDisplacementsOfElement(
        y, res, _factor * _grid.GetElementWeight());
  }
...
template <class Grid> Eigen::VectorXd &GenericMatrix<Grid>::Diagonal() {
  if (_dia == 0) {
    _grid.Recv_export_Ghost();
    ...
    for (_grid.initIterateOverElements(); _grid.TestIterateOverElements();
      ...
      _grid.SearchIndexes();
      _grid.SumInToNodalDisplacementsOfElement(*diagonal, res,
                                               _grid.GetElementWeight());
    }
    ...
    // compute the pseudo invers of the diagonal. If the entry is zero than set
    // it also in the inverse to zero
    for (t_index i = 0; i < _mydofs; ++i) {
...
template <class Grid> int GenericMatrix<Grid>::PrepareApply() {
...
template <class Grid>
void GenericMatrix<Grid>::SetBoundaries(Eigen::VectorXd &y,
                                        Eigen::VectorXd &x) const {
  t_index i = 0;
  std::vector<t_index>::const_iterator it;

  for (it = _grid.bc.FixedNodes_Ind.begin();
       it != _grid.bc.FixedNodes_Ind.end(); ++it) {
    x[*it] = _stored_disp[i++];
    y[*it] = x[*it];
  }
}
...
std::ostream &GenericMatrix<Grid>::print_mat(std::ostream &stream) const {
  for (int i = 0; i < 24; i++) {
    for (int j = 0; j < 24; j++)
      stream << std::setiosflags(std::ios::fixed) << std::setw(7)
             << std::setprecision(4) << (*_stiffnessmatrix)(i, j) << " ";
    stream << std::endl;
  }
  return stream;
}
...
std::ostream &GenericMatrix<Grid>::print_prop(std::ostream &stream) const {
  ...
  stream << "Matrix:\n";
  stream << "   used Grid Size: " << GlobalDim[0] << ", " << GlobalDim[1]
         << ", " << GlobalDim[2] << "\n";
  stream << "   used Grid Resolution: " << GridDim[0] << ", " << GridDim[1]
         << ", " << GridDim[2] << "\n";
  stream << "   used Material properties:   E: " << _matprop[0]
         << " Poisson: " << _matprop[1] << std::endl;
  return stream;
}
