...
template <class T> class PfePrinter {
public:
  PfePrinter(std::string filename, OctreeGrid<T> &grid)
      : _MyPID(grid.GetPID()), _Size(grid.GetNrCPU()), _filename(filename),
        _grid(grid) {
  ...
  void PrintBC() {
    ...
    int n_fixed_nodes;
    MPI_Allreduce(&my_n_fixed_nodes, &n_fixed_nodes, 1, MPI_INT, MPI_SUM,
                  MPI_COMM_WORLD);
    ...
    for (int i = 0; i < my_n_fixed_nodes; i++) {
      ...
      if (abs_bc_val > 1e-14) {
        node_numbers_fixed[myNFixedNodes] =
            _grid.bc.FixedNodes_Ind[i] / 3 + offset + 1;
    ...
    MPI_Scan(&myNFixedNodes, &my_fixed_nodes_offset, 1, MPI_INT, MPI_SUM,
             MPI_COMM_WORLD);
    MPI_Allreduce(&myNFixedNodes, &N_actual_fixed_nodes, 1, MPI_INT, MPI_SUM,
                  MPI_COMM_WORLD);
    ...
    if (N_actual_fixed_nodes > 0) {
      Writer->Write("Fixed nodes", "Node number", node_numbers_fixed, "Sense",
                    sense_fixed, "Value", value_fixed, N_actual_fixed_nodes,
                    myNFixedNodes, 1, 1, 1, my_fixed_nodes_offset);
    }
  ...
  void PrintMaterialIDs(std::string dset) {
    ...
    Writer->Write(dset, MatIDs.data(), _grid.GetNrElemGlobal(),
                  _grid.GetNrElem(), 1, _grid.GetElemOffset());
  }
  ...
  void PrintCoord(std::string dset) {
    ...
    Writer->Write(dset, coord.data(), _grid.GetNrNodesGlobal(),
                  _grid.GetNrPrivateNodes(), 3, _grid.GetNodeOffset());
  }
  ...
  void PrintElems(std::string dset) {
    ...
    Writer->Write(dset, elems, _grid.GetNrElemGlobal(), _grid.GetNrElem(), 8,
                  _grid.GetElemOffset());
    delete[] elems;
  }

  void PrintAll(Eigen::VectorXd &x, Eigen::VectorXd &force,
                Eigen::VectorXd &res) {
    ...
    Writer->Write("Nodal displacements", x.data(), _grid.GetNrNodesGlobal(),
                  _grid.GetNrPrivateNodes(), 3, _grid.GetNodeOffset());
    Writer->Write("VonMises", m.data(), _grid.GetNrElemGlobal(),
                  _grid.GetNrElem(), 1, _grid.GetElemOffset());
    Writer->Write("SED", s.data(), _grid.GetNrElemGlobal(), _grid.GetNrElem(),
                  1, _grid.GetElemOffset());
    Writer->Write("EFF", eff.data(), _grid.GetNrElemGlobal(), _grid.GetNrElem(),
                  1, _grid.GetElemOffset());
    Writer->Write("Element stress", stresses.data(), _grid.GetNrElemGlobal(),
                  _grid.GetNrElem(), 6, _grid.GetElemOffset());
    Writer->Write("Element strain", strains.data(), _grid.GetNrElemGlobal(),
                  _grid.GetNrElem(), 6, _grid.GetElemOffset());
    Writer->Write("Nodal forces", force.data(), _grid.GetNrNodesGlobal(),
                  _grid.GetNrPrivateNodes(), 3, _grid.GetNodeOffset());
  }
