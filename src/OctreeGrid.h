...
/*! This class stores the grid. It also provides an iterator over the grid. The
 * template parameter defines the type of node in the grid. For a implementation
 * see OctreeNode.
 */
template <typename T> class OctreeGrid : public BaseGrid {
public:
  /** @name Constructors/destructors*/
  //@{
  //! OctreeGrid Constructor
  OctreeGrid() { init_Members(); }
  ...
  t_index GetNrDofs() { return _nr_nodes * 3; }

  t_index GetNrNodes() { return _nr_nodes; }

  t_index GetNrPrivateNodes() { return _GridIteratorEnd - _OctreeGrid.begin(); }

  t_index GetNrElem() { return _nr_elem; }

  t_octree_key GetNrElemGlobal() { return _nr_elem_global; }

  t_octree_key GetNrNodesGlobal() { return _nr_nodes_global; }
  t_octree_key GetNodeOffset() { return _my_nodes_offset; }
  t_octree_key GetElemOffset() { return _my_elem_offset; }

  std::vector<OctreeNode> &GetOctGrid() { return _OctreeGrid; }
  ...
  int GetPID() { return _My_PID; }
  int GetNrCPU() { return _Nr_CPU; }
  //@}
   ...
   */
  double GetElementWeight() { return _GridIterator->w; }
  //@}
  ...
  void SearchIndexes0() {
    ...
    for (int i = 1; i < 8; i++) {
      ...
      if (ind < 0) {
        COUT << " ERRRROOOOOORRRRRRRRR!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! on "
                "PID: "
             << _My_PID << std::endl;
        COUT << " Could not find key: " << keys[i] << std::endl;
        COUT << " main key is: " << keys[0] << " weight is "
             << _OctreeGrid[_neighbours[0]].w << std::endl;
      }
      _neighbours[i] = ind;
    }
    return;
  ...
  /**
   * @brief Search the 8 index of the neighbours and stores in the neighbour
   * array member. This is the optimized version.
   */
  void SearchIndexes() {
  ...
  /**
   * @brief Copies the 8x3 nodal values of the actual element in an fixed size
   * vector The iterator must be fixed on the actual element.
   *
   * @param x Source vector
   * @param pref fixed size vector
   */
  void GetNodalDisplacementsOfElement(Eigen::VectorXd &x,
                                      Eigen::Matrix<double, 24, 1> &pref) {
    SearchIndexes();
   ...
   * @param pref fixed size vector with 24 values that are summed into the
   * vector
   * @param factor x+= factor * pref
   */
  void SumInToNodalDisplacementsOfElement(Eigen::VectorXd &x,
                                          Eigen::Matrix<double, 24, 1> &pref,
                                          double factor) {
    for (int i = 0; i < 8; i++) {
  ...
  inline int GetIndexOfKey(t_octree_key key) {
    t_octree_iterator tmp =
        lower_bound(_OctreeGrid.begin(), _OctreeGrid.end(), key);
   ...
   *
   * @param match Starting iterator for the search. After the call the iterator
   * points to the found object.
   ...
   * search in the array above the  match iterator. The key must exist in the
   * vector.
   *
   * @param match Starting iterator for the search. After the call the iterator
   * points to the found object.
   ...
   */
  void GetIndexOfBC(std::vector<t_boundary_node> &key_list,
    ...
    for (iter = key_list.begin(); iter != key_list.end(); ++iter) {
      key = iter->first;
      t_octree_iterator tmp =
          lower_bound(_OctreeGrid.begin(), _GridIteratorEnd, key); // hmhmhm
      if (tmp != _GridIteratorEnd && !(key < *tmp))
        index = tmp - _OctreeGrid.begin();
      else {
        index = -1;
        COUT << "Error! index of BC not found!\n";
      }
  ...
  void Distribute_the_BC(std::vector<t_boundary_node> &_BC_list) {
    ...
    std::vector<std::vector<t_boundary_node>::iterator> send_pos_iters(_Nr_CPU +
                                                                       1);
    ...
    for (int i = 0; i < _Nr_CPU; i++) {
      while ((iter != _BC_list.end()) &&
             (iter->first < _smallest_elements[i + 1]))
        iter++;
    ...
    int newsize = (recv_pos[_Nr_CPU - 1] + recv_count[_Nr_CPU - 1]) /
                  sizeof(t_boundary_node);
    ...
    MPI_Alltoallv(send_addr, &send_count[0], &send_pos[0], MPI_CHAR, recv_addr,
                  &recv_count[0], &recv_pos[0], MPI_CHAR, MPI_COMM_WORLD);
  ...
  void CheckGrid(const char *msg) {
    ...
    for (iter = _OctreeGrid.begin(); iter < _OctreeGrid.end(); ++iter) {
      if (iter->key == 0) {
        std::cout << "Error after " << msg << "  on PID: " << _My_PID
                  << std::endl;
        std::cout.flush();
      }
    }
  ...
  void PrintVector(Eigen::VectorXd &x) {
    ...
    for (int j = 0; j < _Nr_CPU; j++) {
      if (_My_PID == j) {
        COUT << "CPU " << j << " holds " << numberelem << " elements"
             << std::endl;
      }
      MPI_Barrier(MPI_COMM_WORLD);
    }
  ...
  void PrintBC(const char *filename) {
    ...
    int j = 0;
    for (bc_it = bc.FixedNodes_Ind.begin(); bc_it != bc.FixedNodes_Ind.end();
         ++bc_it) {
      // if (Iter->w > 0) {
  ...
  bool CheckIndexOfBC(std::vector<t_boundary_node> &key_list,
                      std::vector<t_boundary_node> &index_list) {
    ...
    for (; list_iter != index_list.end(); ++list_iter, ++BC_iter) {
      if (_OctreeGrid[list_iter->first] != BC_iter->first) {
        COUT << "BC key: " << BC_iter->first << " OCT Key "
             << _OctreeGrid[list_iter->first] << std::endl;
        ret = false;
      }
    }
    return ret;
    ...
    for (int j = 0; j < _Nr_CPU; j++) {
      if (_My_PID == j) {
        for (unsigned r = 0; r < _sending_elements_pid.size(); r++) {
          stream << j + 1 << "\t" << _sending_elements_pid[r] + 1 << "\t"
                 << _sending_elements_pos[r + 1] - _sending_elements_pos[r]
                 << std::endl;
        }
        for (unsigned r = 0; r < _remote_elements_pid.size(); r++) {
          stream << j + 1 << "\t" << _remote_elements_pid[r] + 1 << "\t"
                 << _remote_elements_pos[r + 1] - _remote_elements_pos[r]
                 << std::endl;
        }
      }
      MPI_Barrier(MPI_COMM_WORLD);
  ...
  void Wait_import_Ghost() {
    ...
    MPI_Waitall(_sending_elements_pid.size(), _sending_request,
                _sending_status);
  }
  ...
  void Send_import_Ghost(Eigen::VectorXd &disp) {
    ...
    for (unsigned i = 0; i < _remote_elements_pid.size(); i++) {
      ...
      MPI_Isend(buff, 3 * size, MPI_DOUBLE, _remote_elements_pid[i], 99,
                MPI_COMM_WORLD, &_remote_request[i]);
    }
    return;
  }
  ...
  void Recv_import_Ghost(Eigen::VectorXd &disp) {
    ...
    for (unsigned i = 0; i < _sending_elements_pid.size(); i++) {
      ...
      MPI_Irecv(recvbuf, 3 * size, MPI_DOUBLE, _sending_elements_pid[i], 99,
                MPI_COMM_WORLD, &_sending_request[i]);
    }
    return;
  }
  ...
  void Send_export_Ghost(Eigen::VectorXd &disp) {
    ...
    for (unsigned i = 0; i < _sending_elements_pid.size(); i++) {
      ...
      int size = _sending_elements_pos[i + 1] - _sending_elements_pos[i];
      MPI_Isend(buf, 3 * size, MPI_DOUBLE, _sending_elements_pid[i], 10,
                MPI_COMM_WORLD, &_sending_request[i]);
    }
    return;
  }
  ...
  void Recv_export_Ghost() {
    for (unsigned i = 0; i < _remote_elements_pid.size(); i++) {
      double *buff = &_remote_elements_buf[3 * _remote_elements_pos[i]];
      int size = _remote_elements_pos[i + 1] - _remote_elements_pos[i];
      MPI_Irecv(buff, 3 * size, MPI_DOUBLE, _remote_elements_pid[i], 10,
                MPI_COMM_WORLD, &_remote_request[i]);
    }
    return;
  }
  ...
  /**
   * @brief Waits of all cpu in the export process and sum it into the local
   * nodes
   */
...
#define BLOCK 8192
    ...
    MPI_Allreduce(&localdot, &globaldot, 1, MPI_DOUBLE, MPI_SUM,
                  MPI_COMM_WORLD);
    return globaldot;
  }
  //!@}
...
public:
  ...
  void Compute_min_max(t_octree_key min, t_octree_key max) {
    ...
    MPI_Allreduce(min_max_in, min_max_out, 2, MPI_LONG, MPI_MAX,
                  MPI_COMM_WORLD);
   ...
   * second bucket bucketboarder[1] - bucketboarder[2], (it on all proc the
   * same)
   ...
   */
  bool Refine_Buckets(std::set<t_octree_key> &bucketsboarder,
                      std::vector<t_octree_iterator> &buckets,
                      std::vector<t_index> &bucket_sizes, t_index max_size) {
   ...
   */
  void Compute_equally_distribution(t_octree_key globalsize,
                                    std::vector<t_index> &localsizes) {
    ...
    for (t_index i = 0; i < nr_buckets; i++) {
      if (i == nr_buckets) {
        ...
        localsizes[i] = (t_octree_key)(average_nr_elem * (i + 1)) -
                        (t_octree_key)(average_nr_elem * i);
      }
    }
    return;
  }
  ...
  /**
   * @brief Fill in the target Buckets. It tries to achieve equal size per
   * Bucket.
   *
   * @param[in] histogram The buckets are stored with iterators for the left and
   * right boundary
   * @param[in] histogram_sizes The sizes of the buckets in the histogram.
   * @param[out] buckets The target buckets. They are stored in the same format
   * as the histogram.
   * @param[in,out] bucket_sizes The sizes of the target buckets. As input it
   * holds the sizes to achieve. Finally the actual size of the buckets.
   */
  void Fill_in_the_Buckets(std::vector<t_octree_iterator> &histogram,
                           std::vector<t_index> &histogram_sizes,
                           std::vector<t_octree_iterator> &buckets,
   ...
   */
  void Compute_Nr_Element_in_buckets(std::vector<t_octree_iterator> &buckets,
                                     std::vector<t_index> &bucketsize) {
    ...
    MPI_Allreduce(&bucketsize_in[0], &bucketsize[0], number_bucket, MPI_INT,
   ...
   */
  void Compute_Start_and_End_of_the_Buckets(
  ...
  void Sort() {
    ...
    MPI_Allreduce(&localsize, &globalsize, 1, MPI_LONG, MPI_SUM,
                  MPI_COMM_WORLD);
    ...
    // Create equally sized buckets. Boarders are stored with values in the
    // keyspace The buckets are stored with an iterator on the octree bucket 0
    // goes from buckets[0] up to (without) buckets[1]
    // Initial bucket is all
    std::set<t_octree_key> bucketsboarder;
    bucketsboarder.insert(_min_max[0]); // insert the minimum
    bucketsboarder.insert(
        _min_max[1]); // insert the maximum (boarder is allways without number
    ...
    while (Refine_Buckets(bucketsboarder, buckets, bucket_sizes,
                          max_nr_elem_per_bucket)) {
    ...
    for (int recv_cpu = 0; recv_cpu < Nr_CPU; recv_cpu++) {
      ...
      MPI_Gather(&sending_count, 1, MPI_INT, &recv_count[0], 1, MPI_INT,
      ...
      MPI_Gatherv(sending_addr, sending_count, MPI_BYTE, &newgrid[0],
                  (int *)&recv_count[0], (int *)&recv_pos[0], MPI_BYTE,
                  recv_cpu, MPI_COMM_WORLD);
    }
  ...
  void Add_Missing_Nodes() {
    ...
    std::copy(boundarynodes.begin(), boundarynodes.end(),
              _OctreeGrid.begin() + _nr_nodes);
  ...
  void Update_OctreesHelper() {
    _GridIteratorEnd = lower_bound(_OctreeGrid.begin(), _OctreeGrid.end(),
                                   _smallest_elements[_My_PID + 1]);
    _nr_nodes = _OctreeGrid.size();
    _nr_elem = 0;
    t_octree_iterator iter;
    for (iter = _OctreeGrid.begin(); iter < _GridIteratorEnd; ++iter) {
    ...
    MPI_Allreduce(&local_elems, &_nr_elem_global, 1, MPI_LONG, MPI_SUM,
                  MPI_COMM_WORLD);
    MPI_Scan(&local_elems, &_my_elem_offset, 1, MPI_LONG, MPI_SUM,
             MPI_COMM_WORLD);
    _my_elem_offset -= local_elems; // include it selfs
    local_elems = _GridIteratorEnd - _OctreeGrid.begin();
    MPI_Allreduce(&local_elems, &_nr_nodes_global, 1, MPI_LONG, MPI_SUM,
                  MPI_COMM_WORLD);
    MPI_Scan(&local_elems, &_my_nodes_offset, 1, MPI_LONG, MPI_SUM,
             MPI_COMM_WORLD);
  ...
  void Prepare_Communication() {
    ...
    //			if (_My_PID == 0) {
    //				COUT << "Grid has a globalsize of: " <<globalsize  <<
    //std::endl; 				COUT << "Min-max: " << _min_max[0] << " - " << _min_max[1] <<
    //std::endl << std::flush;
    //			}
    ...
    for (int i = 0; i < _Nr_CPU; i++) {
      data_send[i] = lower_bound(_OctreeGrid.begin(), _OctreeGrid.end(),
                                 _smallest_elements[i]);
    }
    std::vector<t_index> nr_remote_elem_per_cpu(_Nr_CPU);

    // Copy OctreeKeys to send
    std::vector<t_octree_key> keys_of_sending_elem(_OctreeGrid.end() -
                                                   _GridIteratorEnd);
    t_octree_key *sending_addr;

    t_index c = 0;
    for (t_octree_iterator iter = _GridIteratorEnd; iter != _OctreeGrid.end();
         ++iter) {
    ...
    for (int recv_cpu = 0; recv_cpu < _Nr_CPU; recv_cpu++) {
      ...
      MPI_Gather(&nr_elem, 1, MPI_INT, &nr_remote_elem_per_cpu[0], 1, MPI_INT,
                 recv_cpu, MPI_COMM_WORLD);
      ...
      if (_My_PID == recv_cpu) {
        ...
        for (int i = 1; i < _Nr_CPU; i++)
          keys_of_remote_elem_pos[i] =
              keys_of_remote_elem_pos[i - 1] + nr_remote_elem_per_cpu[i - 1];

        nr_elem_recv = keys_of_remote_elem_pos[_Nr_CPU - 1] +
                       nr_remote_elem_per_cpu[_Nr_CPU - 1];
        if (nr_elem_recv > 0) {
      ...
      if (0 == nr_elem) {
        ...
        sending_addr =
            &keys_of_sending_elem[data_send[recv_cpu] - _GridIteratorEnd];
      }

      MPI_Gatherv(sending_addr, nr_elem, MPI_LONG, recv_addr,
                  (int *)&nr_remote_elem_per_cpu[0],
                  (int *)&keys_of_remote_elem_pos[0], MPI_LONG, recv_cpu,
                  MPI_COMM_WORLD);
    }
    ...
    for (key_iter = keys_of_remote_elem.begin();
         key_iter != keys_of_remote_elem.end(); ++key_iter) {
    ...
    inplace_merge(_OctreeGrid.begin(), _OctreeGrid.begin() + o_size,
                  _OctreeGrid.end());
    ...
    Fill_in_pid_and_positions(nr_remote_elem_per_cpu, _remote_elements_pid,
                              _remote_elements_pos);
    ...
    Fill_in_pid_and_positions(nr_sending_elements, _sending_elements_pid,
                              _sending_elements_pos);
...
private:
   ...
   */
  void Copy3(Eigen::VectorXd &x, t_index k, Eigen::Matrix<double, 24, 1> &fix,
             t_index l) {
   ...
   */
  template <class G> int Count_Nr_CPU(const std::vector<G> &nr_elem_per_cpu) {
    int nr_procs = 0;
   ...
   */
  void Fill_in_pid_and_positions(const std::vector<t_index> &nr_elem_per_cpu,
                                 std::vector<int> &elements_pid,
                                 std::vector<t_index> &elements_pos) {
    int k = 0;
    for (unsigned i = 0; i < nr_elem_per_cpu.size(); i++) {
      if (nr_elem_per_cpu[i] > 0) {
        elements_pid[k] = i;
...
template <class T> OctreeGrid<T>::OctreeGrid(ImageReader &ir) {
...
template <class T> void OctreeGrid<T>::init_Members() {
  for (int i = 0; i < 8; i++)
    _neighbours[i] = 0;
...
// only debug reason
template <class T> void OctreeGrid<T>::Print() {
  ...
  COUT << "elemental density: " << ((float)_nr_elem) / ldim_z / ldim_x / ldim_y
       << "\n";
  COUT << "nodal density: "
       << ((float)_nr_nodes) / (ldim_z + 1) / (ldim_x + 1) / (ldim_y + 1)
       << "\n";
...
template <class T> void OctreeGrid<T>::GenerateOctree() {
  // some members
  _nr_elem = 0;
  _nr_nodes = 0;
  if (_grid == 0) {
    ...
    for (long y = 0; y < ldim_y; y++)
      for (long x = 0; x < ldim_x; x++) {
        ...
        if (w > 0) {
          _nr_elem++;
          tmp_node =
              OctreeNode(KeyGen(x + corner_x, y + corner_y, z + corner_z), w);
            ...
            for (long ty = 0; ty < 2; ty++)
              for (long tx = 0; tx < 2; tx++) {
                if (tz + ty + tx == 0) {
                  ...
                  it = ret.first;
                  tmp_node = OctreeNode(
                      KeyGen(tmpx + corner_x, tmpy + corner_y, tmpz + corner_z),
                      -2.0);
                  if (tmpz == ldim_z || tmpy == ldim_y || tmpx == ldim_x ||
                      _grid[tmpz * ldimxy + tmpy * ldim_x + tmpx] == 0) {
                    ++it;
                    tmp_node =
                        OctreeNode(KeyGen(tmpx + corner_x, tmpy + corner_y,
                                          tmpz + corner_z),
                                   -2.0);
  ...
  for (it_coord = fixed_nodes_coordinates.begin();
       it_coord != fixed_nodes_coordinates.end();) {
  ...
  _BC_fixednode_list.insert(_BC_fixednode_list.begin(), bcset.begin(),
                            bcset.end());

  _BC_LoadedNode_list.resize(0);
  bcset.clear();
  it_val = loaded_nodes_values.begin();
  for (it_coord = loaded_nodes_coordinates.begin();
       it_coord != loaded_nodes_coordinates.end();) {
    unsigned short x, y, z, d;
    float disp;
    x = *it_coord;
    ++it_coord;
  ...
  _BC_LoadedNode_list.insert(_BC_LoadedNode_list.begin(), bcset.begin(),
                             bcset.end());
...
std::ostream &OctreeGrid<T>::print(std::ostream &stream) const {
  stream << "OctreeGrid:\n";
  stream << "   local Dimension: " << ldim[0] << ", " << ldim[1] << ", "
         << ldim[2] << "\n";
  stream << "   global Dimension: " << gdim[0] << ", " << gdim[1] << ", "
         << gdim[2] << "\n";
  stream << "   Resolution: " << res[0] << ", " << res[1] << ", " << res[2]
         << "\n";
  stream << "   Global Nr. Nodes: " << _nr_nodes_global
         << " Global Nr. Elements: " << _nr_elem_global << "\n";
  stream << "   elemental density: "
         << ((float)_nr_elem_global) / gdim_z / gdim_x / gdim_y << "\n";
  stream << "   nodal density: "
         << ((float)_nr_nodes_global) / (gdim_z + 1) / (gdim_x + 1) /
                (gdim_y + 1)
         << "\n";
  stream << bc;
  return stream;
