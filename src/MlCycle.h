...
template <typename T> class MlCycle : public Solver {
public:
  /**
   * @brief Constructor of the MG-Algorithm.
   * The matrix must be prepared. This means it must have included the
   * boundaries.
   *
  ...
  MlCycle(OctreeGrid<T> &finegrid, GenericMatrix<OctreeGrid<T>> &mat,
          int degree = 8, int level = -1, int stype = 0, double ratio = 16,
          double w = 0.61);
  ...
  const std::string Label() const { return "Multilevel solver"; }
...
template <class T>
MlCycle<T>::MlCycle(OctreeGrid<T> &finegrid, GenericMatrix<OctreeGrid<T>> &mat,
                    int degree, int level, int stype, double ratio, double w)
    : _finegrid(finegrid), _finemat(mat), _level(level), _w(w) {
  // Debug output
  //	if (mat.GetPID() ==0) {
  //		COUT << "ML Level: " << level << " degree: " << degree << " w: " <<
  //w << "\n";
  //	}
  ...
  if (stype == 0) {
    Chebyshev *tmpsmoother = new Chebyshev(_finemat, degree, true, r_fine,
                                           e_fine, MaxIters, 0, 0, ratio);
    presmoother = tmpsmoother;
    postsmoother = new Chebyshev(*tmpsmoother, false);
  } else {
    JacobiSmoother *tmpsmoother =
        new JacobiSmoother(_finemat, degree + level, w, true, r_fine);
    presmoother = tmpsmoother;
  ...
  if (level == 0 || (ld[0] == 2) || (ld[1] == 2) || (ld[2] == 2))
    ...
    _solver = new MlCycle(*_coarsegrid, *_coarsemat, degree, level - 1, stype,
                          ratio, w);
...
int MlCycle<T>::Solve(Eigen::VectorXd &b, Eigen::VectorXd &x) {
  ...
  debr = b - debr;
  COUT << "(level " << _level << ") "
       << "Res before smoothing " << debr.norm() << std::endl;
  COUT << "(level " << _level << ") "
       << "b  before smoothing " << debr.norm() << std::endl;
#endif
  ...
  debr = b - debr;
  COUT << "(level " << _level << ") "
       << "Res after smoothing " << debr.norm() << std::endl;
#endif
  ...
  for (int cy = 0; cy < 2; cy++) {
...
#ifdef DEBOUT
    COUT << "(level " << _level << ") "
         << "Res in solve after 1. smoothing " << r_fine.norm() << std::endl;
#endif
...
#ifdef DEBOUT
    COUT << "(level " << _level << ") "
         << "e_coarse before prolongate " << e_coarse.norm() << std::endl;
#endif
    // x = x + Prolongate(e)
...
#ifdef DEBOUT
    COUT << "(level " << _level << ") "
         << "e_fine before prolongate " << e_fine.norm() << std::endl;
#endif
#ifdef DEBOUTV
    COUT << "XXXXXX: \n";
    ...
    debr = b - debr;
    COUT << "(level " << _level << ") "
         << "Res before post " << debr.norm() << std::endl;
#endif
    ...
    debr = b - debr;
    COUT << "(level " << _level << ") "
         << "Res after post " << debr.norm() << std::endl;
#endif
  }
