...
public:
   ...
   * @param maxIter Maximal number of iteration of the iterative solving
   * process.
   * @param output If this parameter is set to true, the solver outputs the
   * residual
   * @param freq If output set it writes freq-th iteration the residual
   *
   */
  PCGSolver(StiffnessMatrix &M, Solver &precon, double tol = 1e-10,
            int maxIter = 500, bool output = true, int freq = 4);
  virtual ~PCGSolver();
