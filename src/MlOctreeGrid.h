...
/*! The MlOctreeGrid is base on the standard grid. It adds the restriction and
 * the prolongation operations. The corresponding key of the coarse grid is the
 * fine grid key divided by 8.
 */
template <typename T> class MlOctreeGrid : public OctreeGrid<T> {
public:
...
private:
  ...
  void coarse_member() {
    ...
    for (int i = 0; i < OctreeGrid<T>::_Nr_CPU + 1; i++)
      OctreeGrid<T>::_smallest_elements[i] =
          _finegrid._smallest_elements[i] / 8;
  ...
  void Distribute_Elements(std::set<OctreeNode> &octset) {
    ...
    it = octset.lower_bound(OctreeNode(
        OctreeGrid<T>::_smallest_elements[OctreeGrid<T>::_My_PID + 1], 0));
    OctreeGrid<T>::_OctreeGrid.resize(0);
    OctreeGrid<T>::_OctreeGrid.reserve(octset.size());
    OctreeGrid<T>::_OctreeGrid.insert(OctreeGrid<T>::_OctreeGrid.begin(), it,
                                      octset.end());

    std::vector<OctreeNode> offcpu_elem;

    std::vector<OctreeNode>::iterator Oct_iter =
        OctreeGrid<T>::_OctreeGrid.begin();
    std::vector<std::vector<OctreeNode>::iterator> send_pos_iters(
        OctreeGrid<T>::_Nr_CPU + 1);
    ...
    for (int i = 0; i < OctreeGrid<T>::_Nr_CPU; i++) {
      while ((Oct_iter != OctreeGrid<T>::_OctreeGrid.end()) &&
             (Oct_iter->key < OctreeGrid<T>::_smallest_elements[i + 1]))
        Oct_iter++;
    ...
    int newsize = (recv_pos[OctreeGrid<T>::_Nr_CPU - 1] +
                   recv_count[OctreeGrid<T>::_Nr_CPU - 1]) /
                  sizeof(OctreeNode);
...
template <class T> void MlOctreeGrid<T>::Coarse() {
  coarse_member();
  ...
  // coarse the grid
  for (iter = _finegrid._OctreeGrid.begin(); iter != _finegrid._GridIteratorEnd;
       ++iter) {
    if (iter->w > 0) {
      weight = iter->w / 8;
  ...
  OctreeGrid<T>::_OctreeGrid.reserve(octset.size());
  OctreeGrid<T>::_OctreeGrid.resize(0);
  OctreeGrid<T>::_OctreeGrid.insert(OctreeGrid<T>::_OctreeGrid.begin(),
                                    octset.begin(), octset.end());
  ...
  for (bc_it = _finegrid.bc.FixedNodes_Ind.begin();
       bc_it != _finegrid.bc.FixedNodes_Ind.end(); ++bc_it) { // , ++i) {
  ...
  OctreeGrid<T>::_BC_fixednode_list.insert(
      OctreeGrid<T>::_BC_fixednode_list.begin(), bcset.begin(), bcset.end());
...
void MlOctreeGrid<T>::Restrict(Eigen::VectorXd &fine, Eigen::VectorXd &coarse) {
  ...
  t_octree_key coarsekey =
      -1; // biggest point. the first point is always smaller
  ...
  for (fine_iter = finegrid.begin(); fine_iter != _finegrid._GridIteratorEnd;
       ++fine_iter, fineindex += 3) {
    coarsekey = new_coarsekey = (fine_iter->key >> 3);
    LocalFineCoordinate = fine_iter->key & 7;

    t_octree_key diffx =
        OctreeGrid<T>::KeyGen.incX(new_coarsekey) ^ new_coarsekey;
    t_octree_key diffy =
        OctreeGrid<T>::KeyGen.incY(new_coarsekey) ^ new_coarsekey;
    t_octree_key diffz =
        OctreeGrid<T>::KeyGen.incZ(new_coarsekey) ^ new_coarsekey;
    ...
    for (int z = 0; z <= max_z; z += 4) {
      for (int y = 0; y <= max_y; y += 2) {
        for (int x = 0; x <= max_x; x++) {
          ...
          new_coarseindex =
              OctreeGrid<T>::GetIndexOfKeyA(new_coarse_iter, new_coarsekey);
          for (int i = 0; i < 3; i++) {
...
template <class T>
void MlOctreeGrid<T>::Prolongate(Eigen::VectorXd &coarse,
                                 Eigen::VectorXd &fine) {
